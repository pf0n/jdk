...
#if INCLUDE_JFR
    // Only emit the ObjectCountAfterGC event if marking was completed.
    if (ShenandoahHeap::is_object_count_active() && heap->is_cit_complete()) {
      heap->tracer()->report_object_count_after_gc(heap->get_cit());
    }
#endif // INCLUDE_JFR
